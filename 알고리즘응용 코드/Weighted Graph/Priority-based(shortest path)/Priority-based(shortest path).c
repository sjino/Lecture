...
void shortest_adjlist(node* g[], int V, int start)
{
	int i;
	node* t;
	pq_init();
	...
	if (check[i] == UNSEEN) //i가 아직 방문하지 않은 노드일때 
	{
		...
		while (!pq_empty()) //queue가 비워질 때까지 반복
		{
			...
			for (t = g[i]; t != NULL; t = t->next)//연결리스트 순차 반복
			{
				...
				{
					if (pq_update(heap, t->vertex, -(t->weight + check[i]))) //fringe node 추가하거나 queue(heap)업데이트 
						/////가중치 입력할 때 지금까지 check(가중치 기록)의 값도 같이 더해서 누적된 가중치로 만들어준다.
					{
...
int main()
{
	...
	printf("\nVisit order of Minimum Spanning Tree(Root : %c)\n", start);
	shortest_adjlist(GL, V, name2int(start)); //최소 비용 
	print_tree(parent, V); //부모 - 자식 출력
	print_cost(check, V, start); //비용 출력
